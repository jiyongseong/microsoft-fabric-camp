...
df_fact_sale = spark.read.table("gold_lakehouse.dbo.fact_sale") 
df_dimension_date = spark.read.table("gold_lakehouse.dbo.dimension_date")
df_dimension_city = spark.read.table("gold_lakehouse.dbo.dimension_city")
```
...
%%sql
SELECT * 
FROM dbo.aggregate_sale_by_date_city;
```
...
```python
sale_by_date_employee = spark.sql("""
SELECT
    DD.Date,
...
INNER JOIN dbo.dimension_date     AS DD ON FS.InvoiceDateKey  = DD.Date
INNER JOIN dbo.dimension_employee AS DE ON FS.SalespersonKey = DE.EmployeeKey
GROUP BY
    DD.Date,
    DD.CalendarMonthLabel,
    DD.Day,
...
%%sql
SELECT *
FROM dbo.aggregate_sale_by_date_employee;
```
